...
-- Lưu ý: Function này giả định dữ liệu mẫu. Bạn cần điều chỉnh theo cấu trúc thực tế.
-- =================================================================================

-- ---------------------------------------------------------------------------------
-- Bước 1: Thêm cột hoc_ky vào bảng chi_tiet_hoat_dong_ren_luyen (nếu chưa có)
-- ---------------------------------------------------------------------------------
-- Thực hiện câu lệnh này để thêm cột hoc_ky vào bảng hiện tại:
-- ALTER TABLE chi_tiet_hoat_dong_ren_luyen ADD COLUMN IF NOT EXISTS hoc_ky CHAR(11);
    ...
    IF EXISTS (
        ...
        SELECT 
            ...
            CASE 
                ...
                WHEN COALESCE(SUM(cthdrl.diem), 0) >= 80 THEN 'Tốt'
                WHEN COALESCE(SUM(cthdrl.diem), 0) >= 65 THEN 'Khá'
                WHEN COALESCE(SUM(cthdrl.diem), 0) >= 50 THEN 'Trung bình'
                WHEN COALESCE(SUM(cthdrl.diem), 0) >= 35 THEN 'Yếu'
                ELSE 'Kém'
            END as xep_loai,
